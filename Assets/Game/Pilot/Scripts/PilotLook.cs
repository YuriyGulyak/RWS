...
public class PilotLook : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        var baseAngles = pilotBase.eulerAngles;
        baseAngles.y = Mathf.SmoothDampAngle( baseAngles.y, CalcBaseAngle(), ref horizontalAngleVelocity, smoothTime );
        pilotBase.eulerAngles = baseAngles;
        ...
        var eyesAngles = pilotEyes.eulerAngles;
        eyesAngles.x = Mathf.SmoothDampAngle( MathUtils.WrapAngle180( eyesAngles.x ), CalcEyesAngle(), ref verticalAngleVelocity, smoothTime );
        pilotEyes.eulerAngles = eyesAngles;
    }
