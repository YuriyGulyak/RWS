...
namespace RWS
{
    public class WingPhotonView : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        void FixedUpdate()
        {
            ...
            targetRigidbody.position = Vector3.MoveTowards( targetRigidbody.position, netPosition,
                distance * ( 1f / PhotonNetwork.SerializationRate ) );
            targetRigidbody.rotation = Quaternion.RotateTowards( targetRigidbody.rotation, netRotation,
                angle * ( 1f / PhotonNetwork.SerializationRate ) );
